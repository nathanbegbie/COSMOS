...
def get_scores(v_data, model_name, combined_model):
    """
    ...
    """
    checkpoint = config.torch.load(config.BASE_DIR + 'models_final/' + model_name + '.pt')
    ...
    img_tensor = [config.torch.tensor(img).to(config.device)]
    bboxes = [config.torch.tensor(bbox_list).to(config.device)]
    bbox_classes = [config.torch.tensor(bbox_classes).to(config.device)]

    if config.embed_type != 'use':
        # For Glove, Fasttext embeddings
        cap1_p = text_field.preprocess(cap1)
        cap2_p = text_field.preprocess(cap2)
        embed_c1 = config.torch.stack([text_field.vocab.vectors[text_field.vocab.stoi[x]] for x in cap1_p]).unsqueeze(
            0).to(config.device)
        embed_c2 = config.torch.stack([text_field.vocab.vectors[text_field.vocab.stoi[x]] for x in cap2_p]).unsqueeze(
            0).to(config.device)
    else:
        # For USE embeddings
        embed_c1 = config.torch.tensor(config.use_embed([cap1]).numpy()).to(config.device)
        embed_c2 = config.torch.tensor(config.use_embed([cap2]).numpy()).to(config.device)
    ...
    # Compute Scores
    score_c1 = config.torch.bmm(z_img, z_text_c1).squeeze()
    score_c2 = config.torch.bmm(z_img, z_text_c2).squeeze()

    return score_c1, score_c2


def evaluate_context_with_bbox_overlap(v_data, model_name, combined_model):
    """
    ...
    score_c1, score_c2 = get_scores(v_data, model_name, combined_model)
...
if __name__ == "__main__":
    ...
    for i, v_data in tqdm(enumerate(test_samples)):
        ...
        pred_context = evaluate_context_with_bbox_overlap(v_data, model_name)
